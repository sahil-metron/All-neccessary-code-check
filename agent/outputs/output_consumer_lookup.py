"""
Consumes lookups_jobs from the queue and sends them to output_lookup_senders
"""
import json
import logging
import threading
...
class OutputLookupConsumer(Thread):

    SUPPORTED_QUEUE_ITEMS: [str] = ['lookup_job']

    def __init__(self,
                 group_name: str,
                 consumer_identification: int,
                 content_queue: QueueType,
                 sender_manager_list: OutputSenderManagerListLookup,
        ...
        self.__sender_manager_list: OutputSenderManagerListLookup = sender_manager_list
            ...
            try:
                ...
                if queue_item:
                    ...
                    if queue_item.type not in self.SUPPORTED_QUEUE_ITEMS:
                        ...
                        # Check the status of output_lookup_senders_service
                        if self.__sender_manager_list.service_status() is False:
                            # The service is unavailable
                            log.warning(
                                f'A new <lookup> event has been detected but it could not be consumed by any '
                                f'sender as there are none available -> {queue_item}'
                            )
                        else:
